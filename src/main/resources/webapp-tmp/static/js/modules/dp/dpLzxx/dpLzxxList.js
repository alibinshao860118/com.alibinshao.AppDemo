...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/dp/dpLzxx/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/dp/dpLzxx/insert';
	}else{
		varUrl = ctx+'/dp/dpLzxx/update';
	}
	...
	if($.trim(formData.lzLb)==""){
		fdp.utils.alertWarn("离职类别不能为空，请填写！");
		return false;
	}
	if($.trim(formData.lzSj)==""){
		fdp.utils.alertWarn("离职时间不能为空，请填写！");
		return false;
	}
