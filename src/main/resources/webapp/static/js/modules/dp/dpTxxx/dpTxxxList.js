...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/dp/dpTxxx/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/dp/dpTxxx/insert';
	}else{
		varUrl = ctx+'/dp/dpTxxx/update';
	}
	...
	if($.trim(formData.txYf)==""){
		fdp.utils.alertWarn("退休年份不能为空，请填写！");
		return false;
	}
	if($.trim(formData.txMf)==""){
		fdp.utils.alertWarn("退休月份不能为空，请填写！");
		return false;
	}
