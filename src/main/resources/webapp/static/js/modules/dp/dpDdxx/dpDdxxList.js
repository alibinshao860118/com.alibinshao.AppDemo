...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/dp/dpDdxx/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/dp/dpDdxx/insert';
	}else{
		varUrl = ctx+'/dp/dpDdxx/update';
	}
	...
	if($.trim(formData.ddSj)==""){
		fdp.utils.alertWarn("调动时间不能为空，请填写！");
		return false;
	}
	if($.trim(formData.xorgMc)==""){
		fdp.utils.alertWarn("新部门不能为空，请填写！");
		return false;
	}
