	...
	bindEvent: function(){
        ...
        $(document).on('click', '.sider-nav .pf-menu-title', function() {
            ...
            if($(this).closest('.no-child').size() > 0) {
	        	...
	        	var menuUrl = ctx + $(this).closest('.no-child').data('href');
	        	$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
					title: $(this).find('.sider-nav-title').text(),
					content: '<iframe class="page-iframe" src="'+ ctx + $(this).closest('.no-child').data('href') +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
					closable: true
				});
            }
        ...
        $(document).on('click', '.sider-nav-s > li', function(e){
        	...
        	//$.messager.progress({text:"加载中，请稍侯......"});
        	layer.load(0,{shade: [0.3, '#393D49'] });//0.1透明度的白色背景
        	$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
				title: titleName,
				content: '<iframe class="page-iframe" src="'+ ctx + $(this).data('href') +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
				closable: true
			});
        	
        	$('.page-iframe:visible').load(function () {
        ...
        $('.pf-logout').on('click', function() {
        	layer.confirm("你确定要退出系统？",{icon:3},function(r) {
        			if(r) {
