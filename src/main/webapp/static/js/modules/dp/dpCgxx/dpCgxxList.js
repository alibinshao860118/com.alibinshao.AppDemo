...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/dp/dpCgxx/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/dp/dpCgxx/insert';
	}else{
		varUrl = ctx+'/dp/dpCgxx/update';
	}
	...
	if($.trim(formData.cgLb)==""){
		fdp.utils.alertWarn("出国类别不能为空，请填写！");
		return false;
	}
	if($.trim(formData.cgGb)==""){
		fdp.utils.alertWarn("出国国别不能为空，请填写！");
		return false;
	}
	if($.trim(formData.cgRq)==""){
		fdp.utils.alertWarn("出国日期不能为空，请填写！");
		return false;
	}
	if($.trim(formData.yhgRq)==""){
		fdp.utils.alertWarn("应回国日期不能为空，请填写！");
		return false;
	}
