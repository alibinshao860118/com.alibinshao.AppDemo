...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/dp/dpYctx/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/dp/dpYctx/insert';
	}else{
		varUrl = ctx+'/dp/dpYctx/update';
	}
	...
	if($.trim(formData.ycYs)==""){
		fdp.utils.alertWarn("延迟月数不能为空，请填写！");
		return false;
	}
	if($.trim(formData.ycYy)==""){
		fdp.utils.alertWarn("延迟原因不能为空，请填写！");
		return false;
	}
