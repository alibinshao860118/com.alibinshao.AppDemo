...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/pxgl/pxPxb/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/pxgl/pxPxb/insert';
	}else{
		varUrl = ctx+'/pxgl/pxPxb/update';
	}
	
	var formData = $("#dialogForm").serializeObject();
	
	//TODO: 请根据实际情况编写
	if($.trim(formData.pxbMc)==""){
		fdp.utils.alertWarn("培训班名称不能为空，请填写！");
		return false;
	}
	if($.trim(formData.jhRs)==""){
		fdp.utils.alertWarn("招收人数不能为空，请填写！");
		return false;
	}
	if($.trim(formData.pxDx)==""){
		fdp.utils.alertWarn("讲师姓名不能为空，请填写！");
		return false;
	}
	if($.trim(formData.kkSj)==""){
		fdp.utils.alertWarn("开课时间不能为空，请填写！");
		return false;
	}
	if($.trim(formData.jkSj)==""){
		fdp.utils.alertWarn("结课时间不能为空，请填写！");
		return false;
	}
