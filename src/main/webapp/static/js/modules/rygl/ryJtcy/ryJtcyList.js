...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/rygl/ryJtcy/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/rygl/ryJtcy/insert';
	}else{
		varUrl = ctx+'/rygl/ryJtcy/update';
	}
	
	var formData = $("#dialogForm").serializeObject();
	
	//表单验证
	if($.trim(formData.jtcy)==""){
		fdp.utils.alertWarn("【家庭成员】不能为空，请填写！");
		return false;
	}
	if($.trim(formData.gx)==""){
		fdp.utils.alertWarn("【关系】不能为空，请填写！");
		return false;
	}
