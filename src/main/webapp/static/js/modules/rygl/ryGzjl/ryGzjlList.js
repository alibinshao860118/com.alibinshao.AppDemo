...
function delete_() {
	...
	fdp.utils.confirm("确定要删除选中的数据吗?", function(r) {
		if (r) {
			$.post(ctx+"/rygl/ryGzjl/delete", {ids : ids.join('&') }, function(result) {
				if (result.success) {
...
function okFunction(){
	...
	if($("#dialogForm #id").val() == ''){
		varUrl = ctx+'/rygl/ryGzjl/insert';
	}else{
		varUrl = ctx+'/rygl/ryGzjl/update';
	}
	...
	if($.trim(formData.jsrq)==""){
		fdp.utils.alertWarn("【结束时间】不能为空，请填写！");
		return false;
	}
	if($.trim(formData.gzdw)==""){
		fdp.utils.alertWarn("【工作单位】不能为空，请填写！");
		return false;
	}
	if($.trim(formData.zw)==""){
		fdp.utils.alertWarn("【职务】不能为空，请填写！");
		return false;
	}
	//提交
